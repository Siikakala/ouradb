...
client_ouradb = influxdb_client.InfluxDBClient(
...
def fetch_data(start, end, datatype, pat_data):
    ...
    params = {
        "start_date": f"{start.strftime('%Y-%m-%d')}", 'end_date': f"{end.strftime('%Y-%m-%d')}"}
...
parser = argparse.ArgumentParser(
    description='Post Oura data to Influxdb. Omit --start and --end to process data only for today.')
